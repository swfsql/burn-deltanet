...
//! `burn-mamba`; what is delta-specific here is [`predict`], which the training
...
use burn_deltanet::prelude::{DeltaLatentNet, DeltaLatentNetConfig};
...
pub fn predict(model: &DeltaLatentNet, images_norm: Tensor<4>) -> Tensor<2> {
    ...
    let zscored = images_norm
        ...
        .reshape([n, h * w, 1]);
    let (output, _caches) = model.forward(zscored, None, crate::training::path(), None);
