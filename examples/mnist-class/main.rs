...
//! single-pixel tokens with a GDN-2 model (a classification head on the last
...
//! [Muon](burn_stack::optim) for a fresh training config (the fused `in_proj`
//! is split per sub-projection first). The choice is written into the
//! artifacts' `training_config.json`, so resuming a run keeps it.
//!
//! ```bash
