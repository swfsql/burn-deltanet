...
//! newtype: it adapts the example network to Burn's `TrainStep` /
//! `InferenceStep` via a cross-entropy classification head on the last
//! timestep, and supplies the `MnistModel` seam the shared loops build against.
...
use burn_deltanet::prelude::{DeltaLatentNet, DeltaLatentNetConfig, DeltaPath};
use burn_stack::examples::mnist::classify::{self, MnistModel, epoch_train, epoch_valid};

use crate::model::OUTPUT_SEQUENCE_EXTRA;

/// The delta-rule path used for both training and inference: the chunkwise WY
...
pub struct Wrap(pub DeltaLatentNet);
...
impl Wrap {
    ...
    pub fn forward_classification(
        ...
        let (output, _caches) = model.forward(input.clone(), None, path(), None);
