//! Inference for the register-carousel example: loads the trained model and
//! reports accuracy on each evaluation family, plus a decoded sample sequence.

use crate::AppArgs;
use crate::dataset::{
    CarouselBatcher, CarouselDataset, CarouselItem, EVAL_SEED, Family, IGNORE, NUM_CLASSES,
    NUM_EVAL, SEQ_LENGTH, Turn, render_classes, render_symbols,
};
...
pub fn infer(
    ...
    println!("turn = {turn:?}, chance ≈ {:.1}%", 100.0 / NUM_CLASSES as f32);
    for (name, family) in EVAL_FAMILIES {
        let items: Vec<CarouselItem> =
            CarouselDataset::new(NUM_EVAL, SEQ_LENGTH, family, turn, EVAL_SEED)
                .iter()
        ...
        println!(
            "{name:<7} acc {:6.2}%   ({correct}/{})   per class {}",
