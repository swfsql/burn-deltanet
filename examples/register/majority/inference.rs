//! Inference for the register-majority example: loads the trained model and
//! reports accuracy on each evaluation family, plus a decoded sample sequence.

use crate::AppArgs;
use crate::dataset::{
    EVAL_SEED, Family, IGNORE, NUM_CLASSES, NUM_EVAL, RegisterBatcher, RegisterDataset,
    RegisterItem, SEQ_LENGTH, render_classes, render_symbols,
};
use crate::training::{EVAL_FAMILIES, path};
use burn::{
...
pub fn infer(model_config: DeltaLatentNetConfig, infer_device: Device, app_args: &AppArgs) {
    let model: DeltaLatentNet = app_args
    ...
    for (name, family) in EVAL_FAMILIES {
        let items: Vec<RegisterItem> =
            RegisterDataset::new(NUM_EVAL, SEQ_LENGTH, family, EVAL_SEED)
                .iter()
        ...
        let (output, _caches) = model.forward(batch.inputs, None, path(), None);
        ...
        let target = batch
            ...
            .try_to_vec::<i32>()
            .unwrap();
        // Only the query positions with a loaded file are scored (`dataset::IGNORE`).
        ...
        println!(
            "{name:<8} acc {:6.2}%   ({correct}/{})   per class {}",
            100.0 * correct as f32 / scored.len() as f32,
            scored.len(),
            per_class(&scored),
        );

        if family == Family::Stale {
