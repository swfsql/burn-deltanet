//! Training loop for the register-majority example: builds the dataloaders,
...
use crate::dataset::{
    EVAL_SEED, Family, NUM_CLASSES, NUM_EVAL, NUM_TRAIN, RegisterBatch, RegisterBatcher,
    RegisterDataset, SEQ_LENGTH, TRAIN_SEED,
};
use burn::prelude::*;
use burn::{
...
/// where an un-erasing state fails, so a single averaged number would hide the
...
/// `DeltaPath::Recurrent` computes the identical function, more slowly.
...
type Dataloader = std::sync::Arc<dyn DataLoader<RegisterBatch> + 'static>;
...
pub fn validate_all(
    ...
    for (name, loader) in loaders {
        ...
        println!("  epoch {epoch}, {name:<8} loss {loss:.4}, acc {acc:6.2}%");
    }
}
...
impl Wrap {
    /// Forward the model and score the majority at every **query** position.
    pub fn forward_classification(
        ...
        // Keep only the positions with something to report; the writes and the
        // queries issued before the file is loaded reach neither the loss nor
        // the accuracy (see `dataset::IGNORE`).
