...
/// [`custom::unit_lower_inverse`] is the same function as [`TriSolve::Blocked`]:
/// the same ladder runs inside the node, and its analytic backward
/// `tril(Tᵀ Ḡ Tᵀ, −1)` must reproduce what autodiff derives from the ladder.
#[test]
fn custom_node_matches_blocked_values_and_grads() {
    let device: Device = Default::default();
    let head_scale = 1.0;
    // `size = 1` is skipped: the ladder is empty there, so `T = I` has no
    // dependency on `N` at all and neither solve produces a gradient.
    for size in [2, 3, 5, 8, 16, 64] {
        let batch = 2;
        let n_inner = random_strict_lower(batch, size, 0.3, &device);
        let head =
            Tensor::<3>::random([batch, size, size], Distribution::Normal(0.0, head_scale), &device);

        let run = |custom_node: bool| {
            let n = Param::from_tensor(Tensor::from_inner(n_inner.clone()));
            let n_strict = n.val().tril(-1);
            let t = if custom_node {
                custom::unit_lower_inverse(n_strict)
            } else {
                unit_lower_inverse(n_strict, TriSolve::Blocked)
            };
            let t_inner = t.clone().inner();
            let loss = (t * Tensor::from_inner(head.clone())).sum();
            let grads = loss.backward();
        ...
        let (t_blocked, d_blocked) = run(false);
        let (t_custom, d_custom) = run(true);

        let value_diff = max_abs_diff(t_blocked, t_custom);
        let grad_diff = max_abs_diff(d_blocked, d_custom);
        assert!(value_diff < 1e-5, "size {size}: value mismatch: {value_diff}");
        assert!(grad_diff < 1e-4, "size {size}: grad mismatch: {grad_diff}");
    }
}

/// Rank > 3 goes through the same node (the caller rank-erases to
/// `[flat, size, size]`), on the shape the chunk path actually uses.
#[test]
fn custom_node_matches_blocked_rank5() {
    let device: Device = Default::default();
    let (b, n_chunks, h, l) = (2, 3, 4, 16);
    let n_inner = Tensor::<5>::random(
        [b, n_chunks, h, l, l],
    ...
    let (t_blocked, d_blocked) = run(false);
    let (t_custom, d_custom) = run(true);
    assert!(max_abs_diff(t_blocked, t_custom) < 1e-5);
    assert!(max_abs_diff(d_blocked, d_custom) < 1e-4);
}
