...
/// Configuration / factory for a single [`DeltaProductCache`].
#[derive(Config, Debug)]
pub struct DeltaProductCacheConfig {
    /// Batch size.
    pub batch: usize,
    /// Number of heads.
    pub nheads: usize,
...
impl DeltaProductCacheConfig {
    ...
    pub fn init(&self, device: &Device) -> DeltaProductCache {
        DeltaProductCache {
...
// ---------------------------------------------------------------------------
// DeltaProductCaches  (one entry per virtual layer)
// ---------------------------------------------------------------------------

/// Per-layer caches for a complete DeltaProduct network — one slot per *virtual*
/// layer, which may exceed the number of real weight sets.
#[derive(Module, Debug)]
pub struct DeltaProductCaches {
    /// Per-layer caches.
    pub caches: Vec<DeltaProductCache>,
}

impl DeltaProductCaches {
